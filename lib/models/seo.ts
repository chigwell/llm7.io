...
export function comparisonMetadata(left: PublicModel, right: PublicModel): Metadata {
  const title = `${left.model_id} vs ${right.model_id}: Price and Latest Statistics | LLM7`;
  const description = truncateDescription(`Compare current LLM7 API pricing, published specifications, and latest aggregated LLM7 stability and response-time statistics for ${left.model_id} and ${right.model_id}.`);
