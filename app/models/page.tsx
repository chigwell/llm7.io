...
export const metadata: Metadata = {
  title: "AI Model API Catalogue, Pricing and Statistics | LLM7",
  description: "Browse public LLM7 API models with current pricing, published capabilities, and observed LLM7 request statistics.",
  alternates: { canonical: "https://llm7.io/models/" },
  robots: { index: true, follow: true },
  openGraph: { title: "AI Model API Catalogue, Pricing and Statistics | LLM7", description: "Public LLM7 API model pricing, capabilities, and observed statistics.", url: "https://llm7.io/models/", images: [{ url: "https://llm7.io/generated/og/model-comparison.png", width: 1200, height: 630 }] },
...
function ModelCard({ model }: { model: (typeof publicModels)[number] }) {
  ...
  return (
    <article data-model-card data-name={(model.display_name + " " + model.model_id).toLowerCase()} data-type={model.model_type} data-tier={model.tier ?? ""} data-status={model.status} data-input={model.modalities.input.join(" ")} data-output={model.modalities.output.join(" ")} data-tools={String(Boolean(model.tools_calling))} data-reasoning={String(Boolean(model.reasoning))} data-json={String(Boolean(model.json_mode))} data-stream={String(Boolean(model.stream))} data-price={price} data-context={String(model.context_window.tokens ?? 0)} data-requests={String(stats?.requests_total ?? 0)} data-success={String(stats?.success_rate ?? -1)} data-latency={String(stats?.latency_p95_ms ?? Number.MAX_SAFE_INTEGER)} data-updated={model.updated_at} className="group rounded-2xl border border-border/60 bg-card/55 p-5 shadow-sm backdrop-blur transition-all hover:-translate-y-0.5 hover:border-primary/45 hover:shadow-md">
      ...
      {stats?.requests_total ? <div className="mt-4 grid grid-cols-2 gap-2 border-t border-border/50 pt-4 text-xs"><p><span className="block text-muted-foreground">Success</span><span className="font-medium">{formatRate(stats.success_rate)}</span></p>{stats.latency_observations ? <p><span className="block text-muted-foreground">P95 latency</span><span className="font-medium">{formatMs(stats.latency_p95_ms)}</span></p> : null}</div> : null}
    </article>
  );
}
