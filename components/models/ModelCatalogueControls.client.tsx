...
export default function ModelCatalogueControls({ tiers }: { tiers: string[] }) {
  ...
  return <form className="mb-8 grid gap-4 rounded-2xl border border-border/60 bg-card/55 p-5 shadow-sm backdrop-blur md:grid-cols-4" onSubmit={(event) => event.preventDefault()} aria-label="Filter the model catalogue">
    ...
    <label className="text-sm font-medium">Sort order<select className="mt-2 w-full rounded-xl border border-border/70 bg-background/60 px-3 py-2 text-sm outline-none transition-shadow focus:ring-2 focus:ring-ring/50" defaultValue="name" onChange={(event) => sort(event.target.value)}><option value="name">Display name</option><option value="price">Current price</option><option value="context">Context size</option><option value="requests">30-day requests</option><option value="success">30-day success rate</option><option value="latency">30-day p95 latency</option><option value="updated">Updated date</option></select></label>
  </form>;
}
